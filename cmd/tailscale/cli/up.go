...
func runUp(ctx context.Context, args []string) error {
	...
	if upArgs.runSSH != curPrefs.RunSSH && isSSHOverTailscale() {
		if upArgs.runSSH {
			presentRiskToUser(riskLoseSSH, `You are connected over Tailscale, this action will reroute SSH traffic to Tailscale SSH and will result in your session disconnecting.`)
		} else {
			presentRiskToUser(riskLoseSSH, `You are connected using Tailscale SSH, this action will result in your session disconnecting.`)
		}
	}
